...
async def handler_input_product_field(msg: Message, session: UserSession, state: FSMContext, field_name: str, value,
    ...
    if result is None:
        ...
        if now_state.startswith(EditProductStates.group_name) or filed_config.is_end_field:
            new_message, next_state = await complete_product(state)
    ...
    if new_message is not None:
        await send_message(session, msg.bot, new_message, False)


async def complete_product(state: FSMContext) -> tuple[None | MessageSetting, State]:
    product_service: InputProductService = await state.get_value(FSMKeys.SellerKeys.ADD_PRODUCT_MANAGER)
    # new_message = None
