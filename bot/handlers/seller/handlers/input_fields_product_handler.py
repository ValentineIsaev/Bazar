...
async def process_product_actions(cb: CallbackQuery, state: FSMContext, storage: Storage,
    ...
    if action == 'start':
        ...
        await handler_input_product_field(cb.message, storage, state, 'catalog',
                                          selected_catalog, is_delete_user_message=False)

    elif action == 'send_product':
        await send_message(storage, cb.bot, MessageSetting(text='ожидайте', keyboard=create_callback_inline_keyboard(
            InlineButtonSetting(
    ...
    if new_message is not None:
        await send_message(storage, cb.bot, new_message, is_send_new)


@router.message(StateFilter(AddProductStates.add_name, EditProductStates.EditParam.edit_name))
async def add_name(msg: Message, state: FSMContext, storage: Storage):
    await handler_input_product_field(msg, storage, state, 'name', msg.text)


@router.message(StateFilter(AddProductStates.add_description,
                                   EditProductStates.EditParam.edit_description))
async def add_description(msg: Message, state: FSMContext, storage: Storage):
    await handler_input_product_field(msg, storage, state, 'description', msg.text)
...
async def add_photo(msg: Message, state: FSMContext, storage: Storage, product_manager: ProductManager):
    ...
    if album or is_skip:
        ...
        await handler_input_product_field(msg, storage, product_manager, state, 'media', user_data, False)


@router.message(StateFilter(AddProductStates.add_price, EditProductStates.EditParam.edit_price))
async def add_price(msg: Message, state: FSMContext, storage: Storage, product_manager: ProductManager):
    await handler_input_product_field(msg, storage, product_manager, state, 'price', msg.text)